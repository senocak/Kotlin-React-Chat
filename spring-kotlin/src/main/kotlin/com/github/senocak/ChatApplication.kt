        ...
        log.debug("[ApplicationReadyEvent]: ${event.timeTaken.toSeconds()}")
            .also {
                if (ddl == "create" || ddl == "create-drop") {
                    ...
                    val user1: User = userRepository.save(User(name = "Anıl Şenocak1", email = "anil1@senocak.com", password = defaultPass, roles = arrayListOf(userRole, adminRole)))
                    val user2: User = userRepository.save(User(name = "Anıl Şenocak2", email = "anil2@senocak.com", password = defaultPass, roles = arrayListOf(userRole)))
                    val user3: User = userRepository.save(User(name = "Anıl Şenocak3", email = "anil3@senocak.com", password = defaultPass, roles = arrayListOf(adminRole)))
                    ...
                    val msg5: Message = messageRepository.save(Message(from = user2, to = user3, binary = "from = anil2, to = anil3 binary"))
